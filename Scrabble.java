...
public class Scrabble {
    ...
    public static int wordScore(String word) {
        ...
        for (int i = 0; i < word.length(); i++) {
            cIndex = (int) word.charAt(i) - 97;
            sum += SCRABBLE_LETTER_VALUES[cIndex];
        }
    ...
    public static String createHand() {
        int randomNumber;
        StringBuilder sb = new StringBuilder(MyString.randomStringOfLetters(HAND_SIZE - 2));
        randomNumber = (int) (Math.random() * 8);
        sb.insert(randomNumber, 'a');
        randomNumber = (int) (Math.random() * 9);
        sb.insert(randomNumber, 'e');
        return sb.toString();
    }
    ...
    public static void playHand(String hand) {
        ...
        while (hand.length() > 0) {
            ...
            for (int i = 0; i < inputString.length(); i++) {
                for (int j = 0; j < hand.length(); j++) {
                    if (inputString.charAt(i) == hand.charAt(j)) {
                        counter++;
                        break;
                    }
                }
		...
		if (hand.length() == 0) System.out.println("Ran out of letters. Total score: " + score + " points");
    ...
    public static void playGame() {
        ...
        while (true) {
			...
			else if (inputString.equals("e")) break;
			else System.err.println("uncorrect input: " + inputString);
